...
@section('content')
    <div class="content container-fluid">
        <div class="row">
            <div class="mb-3">
                <h3 class="mb-0">{{__('messages.edit_sale_title')}}</h3>
            </div>
            <div class="col-md-12 offset-md-0 grid-margin stretch-card" id="sale-edit-page">
                <form action="{{ route('sales.update') }}" id="sale-form" enctype="multipart/form-data" method="post">
                    @csrf
                    <div class="card mb-3">
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="payment_type" id="price_type" class="form-control form-select select2" required>
                                        <option value="">{{__('messages.select')}}</option>
                                        <option value="retail_price" @if($sale->payment_type == 'retail_price') selected @endif>Retail Price</option>
                                    </select>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="bonus">{{__('messages.bonus')}} <span class="text-danger">*</span></label>
                                    <div class="mt-2">
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="bonus" id="bonus-yes" value="1" @if($sale->bonus == 1) checked @endif required>
                                            <label class="form-check-label" for="bonus-yes">{{__('messages.yes')}}</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="bonus" id="bonus-no" value="0" @if($sale->bonus == 0) checked @endif required>
                                            <label class="form-check-label" for="bonus-no">{{__('messages.no')}}</label>
                                        </div>
                                    </div>
                                </div>
