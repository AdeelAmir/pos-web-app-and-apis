...
class ReturnController extends Controller
{
    ...
    public function load(Request $request)
    {
        ...
        if ($searchTerm == '') {
            $fetch_data = Returns::join('users', 'users.id', 'returns.seller_id')
                ->select(
                    ...
                    DB::raw('(SELECT products.pieces_in_box FROM products INNER JOIN return_details ON return_details.product_id = products.id WHERE return_details.return_id = returns.id LIMIT 1) as pieces_in_box'),
                )
                ->whereNull('returns.to_city_id')
        ...
        foreach ($fetch_data as $row => $item) {
            $View = route('returns.view', array($item->id));
            $Edit = route('returns.edit', array($item->id));
            $sub_array = array();
            ...
            $sub_array['boxes'] = (!empty($item->return_details_count) && !empty($item->pieces_in_box)) ? SiteHelper::makeBoxes($item->return_details_count, $item->pieces_in_box) : 0;
            ...
            $sub_array['action'] = '
                ...
                <span id="delete||' . $item->id . '" onclick="Delete(' . $item->id . ');" class="text-danger fs-6 cursor-pointer ml-1" data-toggle="tooltip" title="' . __('messages.btns.delete') . '">
    ...
    public function add()
    {
        ...
        return view('admin.returns.add', compact('sellers', 'cities'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->return_grand_total != 0) {
            $returns = new Returns();
            $returns->date = Carbon::parse($request->date)->format('Y-m-d');
            $returns->seller_id = $request->seller_id;
            $returns->city_id = $request->city_id;
            $returns->sale_id = $request->sale_id;
            $returns->grand_total = $request->return_grand_total;
            $affected = $returns->save();
        }

        if ($request->jsonReturnData != '') {
            $products = json_decode($request->jsonReturnData, true);
            foreach ($products as $product) {
                if ($product['return_quantity'] != 0) {
                    ...
                    $returnDetails->return_quantity = $product['return_quantity'];
                    ...
                    $saleDetails->decrement('quantity', $product['return_quantity']);
        ...
        if ($affected) {
            return redirect()->route('returns')->with('success-message', 'Return added successfully');
        } else {
            return redirect()->route('returns')->with('error-message', 'An unhandled error occurred');
        }
    }
    ...
    public function edit($id)
    {
        ...
        // $returnDetails = Returns::join('return_details', 'returns.id', 'return_details.return_id')
        //     ->join('products', 'products.id', 'return_details.product_id')
        //     // ->join('sales', 'sales.id', 'returns.sale_id')
        //     ->join('sale_details', 'returns.sale_id', 'sale_details.sale_id')
        //     ->select(
        //         'return_details.*',
        //         'products.name',
        ...
        //                     WHERE A.date = "' . Carbon::parse($return['date'])->format('Y-m-d') . '"
        ...
        //                         AND C.date = "' . Carbon::parse($return['date'])->format('Y-m-d') . '"
        //                     ),
        //                     0
        //                 )
        ...
        $returnDetails = Returns::join('return_details', 'returns.id', 'return_details.return_id')
            ...
            ->addBinding([Carbon::parse($return['date'])->format('Y-m-d'), Carbon::parse($return['date'])->format('Y-m-d')])
            ->where('returns.id', $id)
            ->get();
        ...
        foreach ($returnDetails as $value) {
            ...
            $array[] = [
                ...
                'boxes' => (!empty($value->remaining_product) && !empty($value->pieces_in_box)) ? SiteHelper::makeBoxes($value->remaining_product, $value->pieces_in_box) : 0,
                'sub_total' => $value->sub_total,
                'return_quantity' => $value->return_quantity
            ];
        }

        $jsonReturnData = json_encode($array);

        return view('admin.returns.edit', compact('return', 'returnDetails', 'sellers', 'cities', 'jsonReturnData'));
    }
    ...
    public function update(Request $request)
    {
            ...
            foreach ($products as $product) {
                ...
                foreach ($dataArray as $data) {
                    if ($data['product_id'] == $product['product_id']) {
                        $newDecrementedQuantity = (int) $product['return_quantity'] - (int) $data['return_quantity'];
        ...
        if ($affected) {
            return redirect()->route('returns')->with('success-message', 'Return updated successfully');
        } else {
            return redirect()->route('returns')->with('error-message', 'An unhandled error occurred');
        }
    }
