...
class CityController extends Controller
{
    ...
    function load(Request $request)
    {
        ...
        if ($searchTerm == '') {
            $fetch_data = City::where('type', 'Warehouse')
                ->orderBy('name', 'asc');
            if ($limit == -1) {
            ...
            $recordsFiltered = City::where('type', 'Warehouse')
                ->orderBy('name', 'asc')
                ->count();
        } else {
            $fetch_data = City::where('type', 'Warehouse')
        ...
        foreach ($fetch_data as $row => $item) {
            $Edit = route('cities.warehouse.edit', array($item->id));
            $sub_array = array();
            $sub_array['id'] = $SrNo;
    ...
    function store(Request $request)
    {
        ...
        if ($Affected) {
            return redirect()->route('cities.warehouse')->with('success-message', 'City added successfully');
        } else {
            return redirect()->route('cities.warehouse')->with('error-message', 'An unhandled error occurred');
        }
    }
    ...
    function update(Request $request)
    {
        ...
        return redirect()->route('cities.warehouse')->with('Success message');
    }
