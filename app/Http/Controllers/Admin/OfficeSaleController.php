...
class OfficeSaleController extends Controller
{
    ...
    {
        return view('admin.office-sales.index');
    }
    ...
    public function load(Request $request)
    {
        ...
        foreach ($fetch_data as $row => $item) {
            $View = route('sales.office.view', array($item->id));
            $Edit = route('sales.office.edit', array($item->id));
            $sub_array = array();
            ...
            $sub_array['boxes'] = (!empty($item->total_quantity) && !empty($item->pieces_in_box)) ? SiteHelper::makeBoxes($item->total_quantity, $item->pieces_in_box) : 0;
            $sub_array['total_total'] = SiteHelper::settings()['Currency_Icon'] . $item->grand_total;
            if ($item->status == 'Pending') {
                $sub_array['payment_status'] = '<span id="' . $item->status . '||' . $item->id . '" onclick="(this.id)" class="btn-sm btn-warning cursor-pointer">Pending</span>';
            } elseif ($item->status == 'Completed') {
                $sub_array['payment_status'] = '<span id="' . $item->status . '||' . $item->id . '" class="btn-sm btn-success cursor-pointer">Completed</span>';
            }
            $sub_array['cash'] = ($item->office_payment_type == 'Cash' && $item->status == 'Completed') ? SiteHelper::settings()['Currency_Icon'] . $item->grand_total : SiteHelper::settings()['Currency_Icon'] . 0;
            $sub_array['loan'] = ($item->office_payment_type == 'Credit' && $item->status == 'Pending') ? SiteHelper::settings()['Currency_Icon'] . $item->grand_total : SiteHelper::settings()['Currency_Icon'] . 0;
            $sub_array['action'] = '
                ...
                <span id="delete||' . $item->id . '" onclick="DeleteOfficeSale(' . $item->id . ');" class="text-danger fs-6 cursor-pointer ml-1" data-toggle="tooltip" title="' . __('messages.btns.delete') . '">
    ...
    public function add()
    {
        ...
        return view('admin.office-sales.add', compact('sellers', 'cities', 'sellingCities'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $sale->selling_city_id = $request->selling_city_id;
        $sale->bonus = 0;
        $sale->grand_total = $request->grand_total;
        $sale->payment_type = $request->payment_type;
        $sale->office_payment_type = $request->office_payment_type;
        $sale->office_payment_type = $request->office_payment_type;
        if ($request->office_payment_type == 'Cash') {
        ...
        if ($affected) {
            return redirect()->route('sales.office')->with('success-message', 'Sale added successfully');
        } else {
            return redirect()->route('sales.office')->with('error-message', 'An unhandled error occurred');
        }
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin.office-sales.edit', compact('sale', 'saleDetails', 'sellers', 'cities', 'sellingCities', 'jsonProducts'));
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($affected) {
            return redirect()->route('sales.office')->with('success-message', 'Sale updated successfully');
        } else {
            return redirect()->route('sales.office')->with('error-message', 'An unhandled error occurred');
        }
    }
    ...
    public function view($id)
    {
        ...
        $saleDetails = SaleDetail::join('products', 'products.id', 'sale_details.product_id')
            ...
            ->where('sale_id', $id)
            ->get();
        return view('admin.office-sales.view', compact('sale', 'seller', 'saleDetails'));
    }
    ...
    public function statusUpdate(Request $request)
    {
        ...
        if ($affected) {
            DB::commit();
            return redirect()->route('sales.office')->with('success-message', 'Office sale status updated successfully');
        } else {
            DB::rollBack();
            return redirect()->route('sales.office')->with('error-message', 'An unhandled error occurred');
        }
